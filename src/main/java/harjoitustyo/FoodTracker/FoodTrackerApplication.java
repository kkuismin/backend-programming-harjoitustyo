...
public class FoodTrackerApplication {
	...
	public CommandLineRunner demo(FoodRepository foodrepository, LocationRepository locationrepository, FoodTypeRepository typerepository) {
		return (args) -> {
			...
			foodrepository.save(new Food("Tomato sauce", 2, "5.10.2023", locationrepository.findBylocationName("Pantry").get(0), typerepository.findByTypeName("Fruits, Vegetables and Berries").get(0)));
			...
			foodrepository.save(new Food("Orange juice", 3, "2.4.2023", locationrepository.findBylocationName("Fridge").get(0), typerepository.findByTypeName("Drinks").get(0)));
